...
public class EasyJoystick : MonoBehaviour
{
    ...
    [SerializeField] private JoystickAnchor joyAnchor = JoystickAnchor.LowerLeft;

    [SerializeField] private Vector2 joystickPositionOffset = Vector2.zero;

    [SerializeField] private float zoneRadius = 100f;

    [SerializeField] private float touchSize = 30f;

    public float deadZone = 20f;

    [SerializeField] private bool restrictArea;
    ...
    [SerializeField] private PropertiesInfluenced xTI;
    ...
    [SerializeField] private float thresholdX = 0.01f;

    [SerializeField] private float stabSpeedX = 20f;
    ...
    [SerializeField] private PropertiesInfluenced yTI;
    ...
    [SerializeField] private float stabSpeedY = 20f;

    public bool enableSmoothing;

    [SerializeField] public Vector2 smoothing = new(2f, 2f);

    public bool enableInertia;

    [SerializeField] public Vector2 inertia = new(100f, 100f);
    ...
    {
        get => new(joystickTouch.x / zoneRadius, joystickTouch.y / zoneRadius);
        set
        {
            var x = Mathf.Clamp(value.x, -1f, 1f) * zoneRadius;
            var y = Mathf.Clamp(value.y, -1f, 1f) * zoneRadius;
    ...
    public float TouchSize
    {
        ...
        {
            touchSize = value;
            if (touchSize > zoneRadius / 2f && restrictArea) touchSize = zoneRadius / 2f;
            ComputeJoystickAnchor(joyAnchor);
        }
    }
    ...
    public InteractionType Interaction
    {
        ...
        {
            interaction = value;
            if (interaction == InteractionType.Direct || interaction == InteractionType.Include) useBroadcast = false;
        }
    }
    ...
    public Vector2 Smoothing
    {
        ...
        {
            smoothing = value;
            if (smoothing.x < 0f) smoothing.x = 0f;
            if (smoothing.y < 0f) smoothing.y = 0f;
        }
    }
    ...
    public Vector2 Inertia
    {
        ...
        {
            inertia = value;
            if (inertia.x <= 0f) inertia.x = 1f;
            if (inertia.y <= 0f) inertia.y = 1f;
        }
    }
    ...
    private void Update()
    {
        if (!useFixedUpdate && enable) UpdateJoystick();
    }
    ...
    private void OnGUI()
    {
        if (enable && visible)
        {
                ...
                (dynamicJoystick && Application.isEditor && !Application.isPlaying))
            {
                ...
                GUI.DrawTexture(
                    new Rect(anchorPosition.x + joystickCenter.x + (joystickTouch.x - touchSize),
                        anchorPosition.y + joystickCenter.y - (joystickTouch.y + touchSize), touchSize * 2f,
                        touchSize * 2f), touchTexture, ScaleMode.ScaleToFit, true);
            }
    ...
    private void UpdateJoystick()
    {
        ...
        {
            joystickIndex = -1;
            if (dynamicJoystick) virtualJoystick = false;
        }

        if (!isActivated) return;
        if (joystickIndex == -1 || (joystickAxis == Vector2.zero && joystickIndex > -1))
        {
            if (enableXAutoStab) DoAutoStabilisation(xAxisTransform, xAI, thresholdX, stabSpeedX, startXLocalAngle);
            if (enableYAutoStab) DoAutoStabilisation(yAxisTransform, yAI, thresholdY, stabSpeedY, startYLocalAngle);
        }

        if (!dynamicJoystick) joystickCenter = joystickPositionOffset;
        if (joystickIndex == -1)
        {
            if (!enableSmoothing)
                joystickTouch = Vector2.zero;
            else if (joystickTouch.sqrMagnitude > 0.0001)
                joystickTouch = new Vector2(joystickTouch.x - joystickTouch.x * smoothing.x * Time.deltaTime,
                    joystickTouch.y - joystickTouch.y * smoothing.y * Time.deltaTime);
            else
                joystickTouch = Vector2.zero;
        }

        var vector = new Vector2(joystickAxis.x, joystickAxis.y);
        var num = ComputeDeadZone();
        joystickAxis = new Vector2(joystickTouch.x * num, joystickTouch.y * num);
        if (inverseXAxis) joystickAxis.x *= -1f;
        if (inverseYAxis) joystickAxis.y *= -1f;
        var vector2 = new Vector2(speed.x * joystickAxis.x, speed.y * joystickAxis.y);
        if (enableInertia)
        {
            var vector3 = vector2 - joystickValue;
            vector3.x /= inertia.x;
            vector3.y /= inertia.y;
        ...
        if (vector == Vector2.zero && joystickAxis != Vector2.zero && interaction != 0 &&
            interaction != InteractionType.Include) CreateEvent(MessageName.On_JoystickMoveStart);
        UpdateGravity();
    ...
    {
        var movingJoystick = new MovingJoystick();
        movingJoystick.joystickName = gameObject.name;
        ...
        if (!useBroadcast)
        {
            switch (message)
            {
                case MessageName.On_JoystickMoveStart:
                    if (On_JoystickMoveStart != null) On_JoystickMoveStart(movingJoystick);
                    break;
                case MessageName.On_JoystickMove:
                    if (On_JoystickMove != null) On_JoystickMove(movingJoystick);
                    break;
                case MessageName.On_JoystickMoveEnd:
                    if (On_JoystickMoveEnd != null) On_JoystickMoveEnd(movingJoystick);
                    break;
                case MessageName.On_JoystickTouchStart:
                    if (On_JoystickTouchStart != null) On_JoystickTouchStart(movingJoystick);
                    break;
                case MessageName.On_JoystickTap:
                    if (On_JoystickTap != null) On_JoystickTap(movingJoystick);
                    break;
                case MessageName.On_JoystickDoubleTap:
                    if (On_JoystickDoubleTap != null) On_JoystickDoubleTap(movingJoystick);
                    break;
                case MessageName.On_JoystickTouchUp:
                    if (On_JoystickTouchUp != null) On_JoystickTouchUp(movingJoystick);
                    break;
            }
        }
        else
            ...
            if (receiverGameObject != null)
                switch (messageMode)
                {
                    case Broadcast.BroadcastMessage:
                        receiverGameObject.BroadcastMessage(message.ToString(), movingJoystick,
                            SendMessageOptions.DontRequireReceiver);
                        break;
                    case Broadcast.SendMessage:
                        receiverGameObject.SendMessage(message.ToString(), movingJoystick,
                            SendMessageOptions.DontRequireReceiver);
                        break;
                    case Broadcast.SendMessageUpwards:
                        receiverGameObject.SendMessageUpwards(message.ToString(), movingJoystick,
                            SendMessageOptions.DontRequireReceiver);
                        break;
                }
            else
    ...
    private void UpdateDirect()
    {
        if (xAxisTransform != null)
        {
            var influencedAxis = GetInfluencedAxis(xAI);
        ...
        if (YAxisTransform != null)
        {
            var influencedAxis2 = GetInfluencedAxis(yAI);
    ...
    private void DoActionDirect(Transform axisTransform, PropertiesInfluenced inlfuencedProperty, Vector3 axis,
        float sensibility, CharacterController charact)
    {
        switch (inlfuencedProperty)
        {
            ...
            case PropertiesInfluenced.Translate:
            {
                ...
                var vector2 = new Vector3(axis.x, axis.y, axis.z);
            ...
            case PropertiesInfluenced.TranslateLocal:
            {
                ...
                var vector = charact.transform.TransformDirection(axis) * sensibility;
    ...
    private void DoAngleLimitation(Transform axisTransform, AxisInfluenced axisInfluenced, float clampMin,
        float clampMax, float startAngle)
    {
        var num = 0f;
        switch (axisInfluenced)
        {
        ...
        if (num <= 360f && num >= 180f) num -= 360f;
        num = Mathf.Clamp(num, 0f - clampMax, clampMin);
        switch (axisInfluenced)
        {
            case AxisInfluenced.X:
                axisTransform.localEulerAngles = new Vector3(num, axisTransform.localEulerAngles.y,
                    axisTransform.localEulerAngles.z);
                break;
            case AxisInfluenced.Y:
                axisTransform.localEulerAngles = new Vector3(axisTransform.localEulerAngles.x, num,
                    axisTransform.localEulerAngles.z);
                break;
            case AxisInfluenced.Z:
                axisTransform.localEulerAngles = new Vector3(axisTransform.localEulerAngles.x,
                    axisTransform.localEulerAngles.y, num);
                break;
        }
    }

    private void DoAutoStabilisation(Transform axisTransform, AxisInfluenced axisInfluenced, float threshold,
        float speed, float startAngle)
    {
        var num = 0f;
        switch (axisInfluenced)
        {
        ...
        if (num > startAngle - threshold || num < startAngle + threshold)
        {
            ...
            if (num < startAngle + threshold) num2 = num + speed / 100f * Mathf.Abs(num - startAngle) * Time.deltaTime;
            switch (axisInfluenced)
            {
                case AxisInfluenced.X:
                    euler = new Vector3(num2, axisTransform.localRotation.eulerAngles.y,
                        axisTransform.localRotation.eulerAngles.z);
                    break;
                case AxisInfluenced.Y:
                    euler = new Vector3(axisTransform.localRotation.eulerAngles.x, num2,
                        axisTransform.localRotation.eulerAngles.z);
                    break;
                case AxisInfluenced.Z:
                    euler = new Vector3(axisTransform.localRotation.eulerAngles.x,
                        axisTransform.localRotation.eulerAngles.y, num2);
                    break;
            }
    ...
    {
        var num = 0f;
        var num2 = Mathf.Max(joystickTouch.magnitude, 0.1f);
        if (restrictArea) return Mathf.Max(num2 - deadZone, 0f) / (zoneRadius - touchSize - deadZone) / num2;
    ...
    private void ComputeJoystickAnchor(JoystickAnchor anchor)
    {
        ...
        switch (anchor)
        {
            ...
            case JoystickAnchor.LowerRight:
                anchorPosition = new Vector2(VirtualScreenbm.width - zoneRadius - num,
                    VirtualScreenbm.height - zoneRadius - num);
                break;
        ...
        areaRect = new Rect(anchorPosition.x + joystickCenter.x - zoneRadius,
            anchorPosition.y + joystickCenter.y - zoneRadius, zoneRadius * 2f, zoneRadius * 2f);
        deadRect = new Rect(anchorPosition.x + joystickCenter.x - deadZone,
            anchorPosition.y + joystickCenter.y - deadZone, deadZone * 2f, deadZone * 2f);
    }

    private void On_TouchStart(Gesturebm gesturebm)
    {
        if (!visible || ((gesturebm.isHoverReservedArea || !dynamicJoystick) && dynamicJoystick) ||
            !isActivated) return;
        if (!dynamicJoystick)
        {
            var vector = new Vector2((anchorPosition.x + joystickCenter.x) * VirtualScreenbm.xRatio,
                (VirtualScreenbm.height - anchorPosition.y - joystickCenter.y) * VirtualScreenbm.yRatio);
            if ((gesturebm.position - vector).sqrMagnitude <
                zoneRadius * VirtualScreenbm.xRatio * (zoneRadius * VirtualScreenbm.xRatio))
            {
        ...
        else
        {
            ...
            switch (area)
            {
                case DynamicArea.FullScreen:
                    ...
                    break;
                case DynamicArea.Bottom:
                    if (gesturebm.position.y < Screen.height / 2) virtualJoystick = true;
                    break;
                case DynamicArea.Top:
                    if (gesturebm.position.y > Screen.height / 2) virtualJoystick = true;
                    break;
                case DynamicArea.Right:
                    if (gesturebm.position.x > Screen.width / 2) virtualJoystick = true;
                    break;
                case DynamicArea.Left:
                    if (gesturebm.position.x < Screen.width / 2) virtualJoystick = true;
                    break;
                case DynamicArea.TopRight:
                    if (gesturebm.position.y > Screen.height / 2 && gesturebm.position.x > Screen.width / 2)
                        virtualJoystick = true;
                    break;
                case DynamicArea.TopLeft:
                    if (gesturebm.position.y > Screen.height / 2 && gesturebm.position.x < Screen.width / 2)
            ...
            if (virtualJoystick)
            {
                joystickCenter = new Vector2(gesturebm.position.x / VirtualScreenbm.xRatio,
                    VirtualScreenbm.height - gesturebm.position.y / VirtualScreenbm.yRatio);
    ...
    private void On_SimpleTap(Gesturebm gesturebm)
    {
        if (visible && ((!gesturebm.isHoverReservedArea && dynamicJoystick) || !dynamicJoystick) && isActivated &&
            gesturebm.fingerIndex == joystickIndex) CreateEvent(MessageName.On_JoystickTap);
    }

    private void On_DoubleTap(Gesturebm gesturebm)
    {
        if (visible && ((!gesturebm.isHoverReservedArea && dynamicJoystick) || !dynamicJoystick) && isActivated &&
            gesturebm.fingerIndex == joystickIndex) CreateEvent(MessageName.On_JoystickDoubleTap);
    }
    ...
    private void On_TouchDown(Gesturebm gesturebm)
    {
        ...
        var vector = new Vector2((anchorPosition.x + joystickCenter.x) * VirtualScreenbm.xRatio,
            (VirtualScreenbm.height - (anchorPosition.y + joystickCenter.y)) * VirtualScreenbm.yRatio);
        if (gesturebm.fingerIndex != joystickIndex) return;
        if (((gesturebm.position - vector).sqrMagnitude <
                zoneRadius * VirtualScreenbm.xRatio * (zoneRadius * VirtualScreenbm.xRatio) && resetFingerExit) ||
            !resetFingerExit)
        {
            joystickTouch = new Vector2(gesturebm.position.x, gesturebm.position.y) - vector;
            joystickTouch = new Vector2(joystickTouch.x / VirtualScreenbm.xRatio,
                joystickTouch.y / VirtualScreenbm.yRatio);
            if (!enableXaxis) joystickTouch.x = 0f;
            if (!enableYaxis) joystickTouch.y = 0f;
