...
public class EasyTouchInput
{
    ...
    private readonly float[] deltaTime = new float[2];

    private Vector2 oldFinger2Position;
    private readonly Vector2[] oldMousePosition = new Vector2[2];

    private readonly float[] startActionTime = new float[2];

    private readonly int[] tapCount = new int[2];

    private readonly float[] tapeTime = new float[2];
    ...
    private int getTouchCount(bool realTouch)
    {
        ...
        else if (Input.GetMouseButton(0) || Input.GetMouseButtonUp(0))
        {
            ...
            if (Input.GetKey(KeyCode.LeftAlt) || Input.GetKey(EasyTouch.instance.twistKey) ||
                Input.GetKey(KeyCode.LeftControl) || Input.GetKey(EasyTouch.instance.swipeKey)) result = 2;
            if (Input.GetKeyUp(KeyCode.LeftAlt) || Input.GetKeyUp(EasyTouch.instance.twistKey) ||
                Input.GetKeyUp(KeyCode.LeftControl) || Input.GetKeyUp(EasyTouch.instance.swipeKey)) result = 2;
        }
    ...
    {
        if (index == 0) return Input.mousePosition;
    ...
    private Vector2 GetPinchTwist2Finger()
    {
        ...
        {
            result.x = Screen.width / 2f - (Input.mousePosition.x - Screen.width / 2f);
            result.y = Screen.height / 2f - (Input.mousePosition.y - Screen.height / 2f);
        }
        else
        {
