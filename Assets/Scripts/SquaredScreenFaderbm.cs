...
public class SquaredScreenFaderbm : Faderbm
{
    ...
    [Range(2f, 50f)] public int columns = 10;
    ...
    {
        base.Init();
        if (texture == null) texture = GetTextureFromColor(color);
        var num = Screen.width + columns;
        var num2 = Screen.height + columns;
        rows = num2 / (num / columns) + 2;
        squares = new AnimRect[columns, rows];
        for (var i = 0; i < columns; i++)
        for (var j = 0; j < rows; j++)
            squares[i, j] = new AnimRect(new Rect(num / columns * i, num2 / rows * j, num / columns, num2 / rows), 0.1f,
                1f);
    ...
    {
        if (columns != last_columns) Init();
        for (var i = 0; i < columns; i++)
        for (var j = 0; j < rows; j++)
            switch (direction)
            {
                case Direction.DIAGONAL_LEFT_DOWN:
                    GUI.DrawTexture(squares[i, j].GetRect(fadeBalance / ((i + j) / (float)(columns + rows))), texture);
                    break;
                case Direction.DIAGONAL_LEFT_UP:
                    GUI.DrawTexture(
                        squares[columns - i - 1, rows - j - 1]
                            .GetRect(fadeBalance / ((i + j) / (float)(columns + rows))), texture);
                    break;
                case Direction.DIAGONAL_RIGHT_DOWN:
                    GUI.DrawTexture(
                        squares[columns - i - 1, j].GetRect(fadeBalance / ((i + j) / (float)(columns + rows))),
                        texture);
                    break;
                case Direction.DIAGONAL_RIGHT_UP:
                    GUI.DrawTexture(squares[i, rows - j - 1].GetRect(fadeBalance / ((i + j) / (float)(columns + rows))),
                        texture);
                    break;
    ...
    private struct AnimRect
    {
        ...
        {
            if (time >= 1f) return rect;
            if (time < 0f)
                return new Rect(rect.xMin + rect.width * time / 2f, rect.yMin + rect.height * time / 2f, 0f, 0f);
            return new Rect(rect.x + rect.width / 2f * (0.5f - time / 2f),
                rect.y + rect.height / 2f * (0.5f - time / 2f), rect.width * time, rect.height * time);
        }
    }
}