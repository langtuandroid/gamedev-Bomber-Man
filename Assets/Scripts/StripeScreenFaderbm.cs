...
public class StripeScreenFaderbm : Faderbm
{
    ...
    [Range(2f, 50f)] public int numberOfStripes = 10;
    ...
    protected override void Init()
    {
        ...
        var num = Screen.width / rcs.Length * 3;
        for (var i = 0; i < rcs.Length; i++)
            rcs[i] = new AnimRect(
                new Rect((Screen.width + num) / rcs.Length * i - 5, -5f, (Screen.width + num) / rcs.Length,
                    Screen.height + 10), 0.1f, 1f);
        last_color = color;
    ...
    protected override void DrawOnGUI()
    {
        for (var i = 0; i < rcs.Length; i++)
        {
            ...
            if ((direction == Direction.HORIZONTAL_IN) | (direction == Direction.HORIZONTAL_OUT) &&
                i > rcs.Length / 2 + 1) break;
        }
    }
    ...
    private struct AnimRect
    {
        ...
        public Rect GetRect(float time)
        {
            ...
            return new Rect(rect.xMin + (rect.width - rect.width * time) / 2f,
                rect.yMin + (rect.height - rect.height * time) / 2f * time, rect.width * time, rect.height * time);
        }
    }
}